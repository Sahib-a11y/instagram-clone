...
const Layout = ({ children, onNavigate, activeTab }) => {
  ...
  const getNavButtonClasses = (tabName) => {
    return `flex flex-col items-center p-2 rounded-lg transition-all duration-200 ${
      activeTab === tabName 
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-900 text-gray-100">
      {/* Main Content */}
      <main className="flex-grow pb-16"> {/* Space for footer */}
        {children}
      </main>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-start justify-center pt-20">
          <div className="bg-gray-800 rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl">
            ...
            <button
              ...
              className="w-full mt-4 px-4 py-3 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Search
            </button>
          </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-gray-800 border-t border-gray-700 z-40">
        <div className="flex justify-around items-center py-2 px-2">
          ...
          <button
            ...
            className="flex flex-col items-center p-2 rounded-lg text-red-500 hover:text-red-400 hover:bg-red-900/20 transition-all"
          >
